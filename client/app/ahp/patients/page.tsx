...
export default function AdminPatientsPage() {
  ...
  const columns: Column<Patient>[] = [
    {
      ...
      render: (patient) => (
        <button
          onClick={() => router.push(`/ahp/patients/${patient.id}`)}
  ...
  const handlePatientAction = (action: string, patientId: string) => {
    ...
    switch (action) {
      case "edit":
        router.push(`/ahp/patients/${patientId}`);
        break;
      case "newTask":
        router.push(`/ahp/tasks/0?patientId=${patientId}`);
        break;
      case "refer":
        // TODO: Open ReferralModal (select target department + note)
        router.push(`/ahp/referrals/0?patientId=${patientId}`);
        break;
      case "delete":
        if (patient) {
          setActionDialog({
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={() => router.push("/ahp/patients/0")}
