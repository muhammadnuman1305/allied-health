...
const createPatientFormSchema = () =>
  z
    .object({
      ...
      gender: z
        .number()
        .min(0, "Invalid gender value")
        .max(2, "Invalid gender value"),
      primaryPhone: z
        .string()
        .optional()
        .refine((val) => {
    ...
    .refine(
      (data) => {
        // At least one emergency contact method must be provided
        const hasPhone =
          data.emergencyContactPhone &&
          data.emergencyContactPhone.trim() !== "";
        const hasEmail =
          data.emergencyContactEmail &&
          data.emergencyContactEmail.trim() !== "";
        return hasPhone || hasEmail;
      },
      {
        message:
          "At least one emergency contact method (phone or email) must be provided",
        path: ["root"], // This will make the error available at errors.root
      }
    );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  {errors.root && !errors.emergencyContactEmail && (
                    <p className="text-sm text-destructive">
                      {errors.root.message}
                    </p>
                  )}
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push("/ahp/patients")}
              disabled={isLoading}
            >
              Cancel
            </Button>
