...
export default function OutgoingReferralsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        toast({ variant: "destructive", title: "Error", description: "Failed to fetch outgoing referrals. Please try again." });
    ...
    () => [
      {
        ...
        render: (referral) => (
          <button
            onClick={() => router.push(`/ahp/patients/${referral.patientId}`)}
      ...
      },
      {
        ...
        render: (referral) => (
          <button
            onClick={() =>
              router.push(`/ahp/setup/departments/${referral.originDeptId}`)
            }
      ...
      },
      {
        ...
        render: (referral) => (
          <button
            onClick={() =>
              router.push(
                `/ahp/setup/departments/${referral.destinationDeptId}`
              )
            }
            className="text-primary hover:underline text-left"
          >
      ...
      },
      {
        ...
        render: (referral) =>
          referral.therapistId && referral.therapistName ? (
            <button
              onClick={() =>
                router.push(`/ahp/users/${referral.therapistId}`)
              }
              className="text-primary hover:underline text-left"
            >
  ...
  const handleReferralAction = (action: string, referralId: string) => {
    switch (action) {
      case "edit":
        router.push(`/ahp/referrals/${referralId}`);
        break;
      case "delete":
  ...
  const handleCompleteReferral = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Error", description: "Failed to complete referral. Please try again." });
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={() => router.push("/ahp/referrals/0")}
