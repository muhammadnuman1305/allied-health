...
import { ReferralFormData, Referral, SelectedComponentInput } from "@/lib/api/admin/referrals/_model";
...
export default function ReferralFormContent({
  ...
  const [isOriginDepartmentDisabled, setIsOriginDepartmentDisabled] =
    useState(false);
  const [interventionComponents, setInterventionComponents] = useState<
    Record<string, SelectedComponentInput[]>
  >({});
    ...
    const loadReferral = async () => {
      if (!isNewReferral) {
        try {
          ...
          setInterventionComponents(referralData.interventionComponents || {});
          // Department workflow fields
          ...
          router.push("/ahp/referrals/outgoing");
        } finally {
                        ...
                        <div key={specialty.id} className="space-y-2">
                          ...
                          <div className="space-y-2">
                            {specialtyInterventions.map((intervention) => {
                              const isSelected = watchedInterventions.includes(
                                intervention.id
                              );
                              const selectedComps =
                                interventionComponents[intervention.id] ?? [];
                              const totalCompTypes = (
                              ...
                              return (
                                ...
                                >
                                  {/* Header row — click to toggle selection (div avoids button-in-button nesting) */}
                                  <div
                                    role="button"
                                    tabIndex={0}
                                    ...
                                    onClick={() => {
                                      ...
                                      if (isSelected) {
                                        ...
                                        // Remove components for this intervention
                                        const next = {
                                          ...interventionComponents,
                                        };
                                        delete next[intervention.id];
                                        setInterventionComponents(next);
                                      } else {
                                  ...
                                  >
                                    ...
                                    {isSelected ? (
                                      <span className="text-xs text-primary font-medium">
                                        ...
                                        {totalCompTypes > 0
                                          ? intervention.components?.reduce(
                                              (sum, g) =>
                                                sum + g.values.length,
                                              0
                                            )
                                      ...
                                      <span className="text-xs text-muted-foreground">
                                        {totalCompTypes} component type
                                        {totalCompTypes !== 1 ? "s" : ""}
                                      </span>
                                    ) : null}
                                  </div>

                                  {/* Component checkboxes — only when selected and has components */}
                                  {isSelected &&
                                    intervention.components &&
                                    intervention.components.length > 0 && (
