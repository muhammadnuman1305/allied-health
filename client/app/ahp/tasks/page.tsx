...
import { formatTableDate, formatRelativeTime } from "@/lib/utils";
...
export default function AdminTasksPage() {
  ...
  const columns: Column<Task>[] = [
    ...
    },
    {
      ...
      render: (task) => (
        <div>
          <button
            onClick={() => router.push(`/ahp/patients/${task.patientId}`)}
      ...
      render: (task) => {
        ...
        return (
          <button
            onClick={() => {
              if (departmentId) {
                router.push(`/ahp/setup/departments/${departmentId}`);
              }
            }}
            className={`text-left ${
    ...
    },
    {
      ...
      width: "w-[140px]",
      sortable: true,
      render: (task) => <div>{formatTableDate(task.startDate)}</div>,
    },
    {
      key: "endDate",
      label: "End",
  ...
  const handleTaskAction = (action: string, taskId: string) => {
    ...
    switch (action) {
      case "edit":
        router.push(`/ahp/tasks/${taskId}`);
        break;
      case "view-patient":
        if (task) {
          router.push(`/ahp/patients/${task.patientId}`);
        }
        break;
      case "start":
        if (task) {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={() => router.push("/ahp/tasks/0")}
