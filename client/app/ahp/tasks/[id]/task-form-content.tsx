...
export default function TaskFormContent() {
  ...
  // Per-intervention selected component values (key = interventionId)
  const [interventionComponents, setInterventionComponents] =
    useState<Record<string, SelectedComponentInput[]>>({});
    ...
    if (!isNewTask && !hasFetchedTask.current) {
      ...
      const fetchTask = async () => {
        try {
          ...
          if (taskWithInterventions.interventionComponents) {
            setInterventionComponents(
              taskWithInterventions.interventionComponents
            );
          }
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-6">
              {/* Intervention Selection — expandable cards grouped by specialty */}
                    ...
                    const interventionsBySpecialty = specialties.reduce(
                      (acc, specialty) => {
                        const specialtyInterventions = interventions.filter(
                          (i) => i.specialtyId === specialty.id
                        );
                        if (specialtyInterventions.length > 0) {
                          acc[specialty.id] = { specialty, interventions: specialtyInterventions };
                        }
                        return acc;
                      },
                      {} as Record<string, { specialty: TaskSpecialty; interventions: TaskIntervention[] }>
                    );
                    ...
                    return Object.values(interventionsBySpecialty).map(
                      ({ specialty, interventions: specInterventions }) => (
                        <div key={specialty.id} className="space-y-2">
                          ...
                          <div className="space-y-2">
                            {specInterventions.map((intervention) => {
                                    ...
                                    onClick={() => {
                                      if (isSelected) {
                                        setSelectedInterventions(
                                          selectedInterventions.filter((i) => i !== intervention.id)
                                        );
                                        const a = { ...interventionAssignments };
                                        delete a[intervention.id];
                                        setInterventionAssignments(a);
                                        const w = { ...interventionWardAssignments };
                                        delete w[intervention.id];
                                        setInterventionWardAssignments(w);
                                        const s = { ...interventionSchedules };
                                        delete s[intervention.id];
                                        setInterventionSchedules(s);
                                        const c = { ...interventionComponents };
                                        delete c[intervention.id];
                                        setInterventionComponents(c);
                                      } else {
                                        if (!selectedInterventions.includes(intervention.id)) {
                                          setSelectedInterventions([...selectedInterventions, intervention.id]);
                                        }
                                      }
                                    }}
                                  >
                                    ...
                                    {intervention.components && intervention.components.length > 0 && (
                                      <span className="text-xs text-muted-foreground">
                                        {isSelected
                                          ? `${(interventionComponents[intervention.id] ?? []).length} / ${intervention.components.flatMap(g => g.values).length} selected`
                                          : `${intervention.components.length} component type${intervention.components.length !== 1 ? "s" : ""}`
                                        }
                                      </span>
                                    )}
                                  </button>

                                  {/* Component checkboxes — only shown when intervention is selected */}
                                  {isSelected && intervention.components && intervention.components.length > 0 && (
                                    <div className="px-4 pb-3 border-t bg-muted/20">
                                      <p className="text-xs text-muted-foreground pt-2 mb-2 font-medium">
                                        Select components <span className="font-normal">(optional)</span>
                                      </p>
                                            ...
                                            <div className="flex flex-wrap gap-x-5 gap-y-1.5">
                                              {group.values.map((val) => {
                                                const isChecked = (interventionComponents[intervention.id] ?? [])
                                                  .some(c => c.componentType === group.type && c.value === val);
                                                return (
                                                  <label
                                                    key={val}
                                                  ...
                                                  >
                                                    <Checkbox
                                                      ...
                                                      onCheckedChange={(checked) => {
                                                        const current = interventionComponents[intervention.id] ?? [];
                                                        const updated = checked
                                                          ? [...current, { componentType: group.type, value: val }]
                                                          : current.filter(
                                                              c => !(c.componentType === group.type && c.value === val)
                                                            );
                                                        setInterventionComponents({
                                                          ...interventionComponents,
                                                          [intervention.id]: updated,
                                                        });
                                                      }}
                                                    />
                                                    {val}
                              ...
                              <div className="flex items-center justify-between gap-3 mb-2">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    delete newComponents[interventionId];
                                    setInterventionComponents(newComponents);
                                  }}
